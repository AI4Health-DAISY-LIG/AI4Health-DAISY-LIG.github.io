...
export function Card({
  ...
  const effectiveVariant = (variant === 'image' && !image) ? 'normal' : variant; 
  ...
  const content = (
    <div
      className={clsx(
        'group relative flex overflow-hidden rounded-sm border border-primary/15 bg-secondary/55 shadow-sm transition-all duration-300 ease-in-out hover:-translate-y-1 hover:border-primary/35 hover:bg-secondary/80 hover:shadow-lg',
        effectiveVariant === 'small'                                                                                                                                                          
      ...
      {external && href && variant !== 'image' && (
        <div
          className={clsx(
            ...
            effectiveVariant === 'small' ? 'right-0' : 'right-2'
          )}
        >
      ...
      <div>
        ...
        <div
          className={clsx(
            ...
            effectiveVariant === 'small' ? 'text-sm' : effectiveVariant === 'image' ? 'p-4 text-sm' : 'text-lg',
            className
          )}
        >
          {title}
        </div>
        {description && effectiveVariant === 'normal' && (
